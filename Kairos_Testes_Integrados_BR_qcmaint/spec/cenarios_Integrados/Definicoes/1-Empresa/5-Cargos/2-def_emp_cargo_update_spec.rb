describe 'Cargos', :cargo_update do
    ...
    it 'Alteração de cargo', :cargo_update_direto do
      within('table tbody tr' , text: 'abcgdga') do
        ...
        selecionar.hover  
        find('span[style="margin-left:4px;margin-top:8px;float:left;"]', text: 'Alterar').click
      end
        
      fill_in 'Cargo_Codigo', with: '101'
      fill_in 'Cargo_Descricao', with: 'cargo alterado'
      drop = find('label[class="pointer"]', text: 'Alterar', match: :first) # precisa manter o match para identificar o botão
      drop.click
